...
const ForcePasswordChange = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Confirmar no banco que a senha foi alterada (remove a flag) se for forçado
      const { error: rpcError } = await chamarApiPost('/api/rpc/confirm_password_change');
      
      if (rpcError) {
        console.error("Erro ao confirmar troca de senha no DB:", rpcError);
        throw new Error("Senha alterada, mas falhou ao atualizar o perfil. Contate o suporte.");
      }
